...
class ActionSheet extends PureComponent {
  static defaultProps = {
    ...
    onPress: () => {},
    styles: {},
    tintColor: Platform.OS === 'ios' ? '#007AFF' : '#F5F5F5'
  }
  ...
  _createButton(title, index) {
    ...
    const buttonText = {
      ...
      fontWeight:
        cancelButtonIndex === index && Platform.os !== 'ios' ? 'bold' : 'normal'
    }
    const borderRadius = options.length - (Platform.OS === 'ios' ? 2 : 1) ===
      index && {
    ...
    return (
      <TouchableHighlight
        ...
        style={[buttonBoxStyle, borderRadius]}
  ...
  render() {
    const styles = this.styles
    const { sheetAnim, visible } = this.state
    return (
      <Modal
        visible={visible}
        animationType="none"
